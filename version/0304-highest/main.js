...
function loadPosts(page, perPage, filter, clearExisting = false, callback = null) {
    ...
    xhr.open('POST', 'version_highest.php', true);
...
function initCarousel() {
    ...
    if (section2) {
        section2.addEventListener('mouseenter', stopCarouselTimer);
        section2.addEventListener('mouseleave', startCarouselTimer);
    }
...
function initModalEvents() {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            if (!/^\d{10}$/.test(id)) {
                if (loginTip) {
                    loginTip.textContent = 'ID必须是10位数字';
                    loginTip.classList.remove('success-tip');
                }
                return;
            }
...
function initTabEvents() {
    ...
    const emoTab = document.getElementById('emoTab');
    const logTab = document.getElementById('logTab');
    const refreshBtn = document.getElementById('refreshBtn');
    const currentTabText = document.getElementById('currentTab');
    
    if (!latestTab || !lifeTab || !codeTab || !studyTab || !emoTab || !logTab) return;
    ...
    // 标签点击
    emoTab.addEventListener('click', function() {
        if (currentTab === 'emo') {
            refreshPosts();
            return;
        }
    ...
    function updateTabUI() {
        ...
        lifeTab.classList.toggle('active', currentTab === 'life');
        codeTab.classList.toggle('active', currentTab === 'code');
        studyTab.classList.toggle('active', currentTab === 'study');
        emoTab.classList.toggle('active', currentTab === 'emo');
        logTab.classList.toggle('active', currentTab === 'log');
        if (currentTabText) {
