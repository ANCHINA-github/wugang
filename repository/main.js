...
// 更多下拉菜单切换（使用事件委托）
document.addEventListener('click', function(e) {
    const toggleBtn = e.target.closest('.more-toggle-btn');
    if (toggleBtn) {
        e.stopPropagation();
        const wrapper = toggleBtn.closest('.more-actions-wrapper');
        if (!wrapper) return;
...
// 点击其他区域关闭所有下拉菜单（点击页面空白处关闭）
document.addEventListener('click', function(e) {
    if (!e.target.closest('.more-actions-wrapper')) {
        document.querySelectorAll('.more-dropdown.open').forEach(function(dropdown) {
...
if (urlPid) {
    // URL 参数优先，覆盖 session 存储
    restoreTargetPid = urlPid;
    isRestoringPosition = true;
    // 写入 session，便于刷新后保持该位置
    sessionStorage.setItem('lastViewedPostId', urlPid);
    // 隐藏帖子列表
...
} else if (sessionLastViewedPid) {
    // 没有 URL 参数，恢复 session 中保存的位置
...
function openCommentsModal(pid) {
    ...
    // 设置模态框内伪输入框的 pid
    const fakeInput = document.getElementById('modalFakeInput');
    if (fakeInput) fakeInput.setAttribute('data-pid', pid);
    ...
    .then(result => {
        if (result.status === 'success') {
            ...
            renderModalComments(comments, pid);
        } else {
            listContainer.innerHTML = '<div style="text-align:center;padding:20px;color:#999;">加载评论失败，请稍后重试</div>';
        }
    })
    .catch(() => {
        listContainer.innerHTML = '<div style="text-align:center;padding:20px;color:#999;">网络错误，请检查连接</div>';
    });
}
    ...
    let html = `
        <div class="post-card" style="box-shadow: none; cursor: default;">
            <div class="post-header">
                <img src="${portrait || 'default-avatar.png'}" alt="用户头像" class="post-avatar" draggable="false" onerror="this.src='default-avatar.jpg'">
                <div class="post-user-info">
                    <div class="post-username">
                        ${pname}
                        ${pname.includes('元宝') || pname.includes('豆包') ? 
                            '<span class="post-badge badge-auth"><i class="fas fa-check-circle"></i> AI</span>' : ''}
                    </div>
                    <div class="post-date">
                        ${pdate}
        ...
        images.forEach((image, index) => {
            ...
            html += `
                <div class="post-image-item ${imageClass}" data-image-src="${image}" data-index="${index}">
                    <img src="${image}" alt="帖子图片" class="post-image" onerror="this.src='a.svg'" draggable="false">
                </div>
            `;
        });
        html += `</div>`;
...
// 初始化评论折叠功能
//initCommentsCollapse中处理了评论区域点击回复的逻辑，现在评论移至模态框，所以该函数可以保留，但其中的 expand-comments-btn 和评论项点击事件不再生效（因为元素已删除）。可以保留，不影响模态框内新绑定的事件。
...
function initImageViewer() {
    ...
    viewerImage.addEventListener('wheel', function(e) {
        ...
        viewerImage.style.transition = 'transform 0.1s ease';
    }, { passive: false });
    ...
    viewerImage.addEventListener('touchmove', function(e) {
        if (e.touches.length === 2) {
            ...
            currentScale = Math.min(MAX_SCALE, Math.max(MIN_SCALE, initialScale * scaleFactor));
            viewerImage.style.transform = `scale(${currentScale})`;
            viewerImage.style.transition = 'none'; // 平滑连续缩放
            e.preventDefault();
        }
        // 单指滑动不在这里处理，在 touchend 中处理
    }, { passive: false });
        ...
        if (currentViewerImages.length > 0 && currentViewerIndex >= 0 && currentViewerIndex < currentViewerImages.length) {
            ...
            if (imageViewerHint) {
                if (currentViewerImages.length > 1) {
                    imageViewerHint.textContent = '提示：双击关闭 | 滚轮缩放 | 滑动切换';
                } else {
                    imageViewerHint.textContent = '提示：双击关闭 | 滚轮缩放';
                }
            }
        }
    }
...
function openImageViewer(images, startIndex = 0) {
    ...
    if (currentViewerImages.length <= 1) {
        ...
        if (imageViewerHint) imageViewerHint.textContent = '提示：双击关闭 | 滚轮缩放';
    } else {
        if (prevImageBtn) prevImageBtn.style.display = 'flex';
        if (nextImageBtn) nextImageBtn.style.display = 'flex';
        if (imageViewerHint) imageViewerHint.textContent = '提示：双击关闭 | 滚轮缩放 | 滑动切换';
    }
...
document.addEventListener('click', function(e) {
    ...
    // 评论显示模态框关闭
const commentshowModalClose = document.getElementById('commentshowModalClose');
if (commentshowModalClose) {
    commentshowModalClose.addEventListener('click', function() {
        hideModal('commentshowModal');
    });
}
// 点击遮罩关闭
const commentshowModal = document.getElementById('commentshowModal');
if (commentshowModal) {
    commentshowModal.addEventListener('click', function(e) {
        if (e.target === this) {
            hideModal('commentshowModal');
        }
    });
...
function renderModalComments(comments, pid) {
    const container = document.getElementById('modalCommentsList');
    if (!container) return;

    if (!comments || comments.length === 0) {
        container.innerHTML = '<div style="text-align:center;padding:20px;color:#999;">还没有评论，快来抢沙发吧~</div>';
        return;
    }
    ...
    // 绑定评论图片预览
    container.querySelectorAll('.comment-image-item').forEach(item => {
        item.addEventListener('click', function(e) {
            e.stopPropagation();
            const src = this.getAttribute('data-image-src');
            const parentContainer = this.closest('.comment-images-container');
            if (!parentContainer) return;
            const items = parentContainer.querySelectorAll('.comment-image-item');
            const images = Array.from(items).map(el => el.getAttribute('data-image-src'));
            const idx = images.indexOf(src);
            if (idx !== -1) openImageViewer(images, idx);
        });
    });
    ...
    container.querySelectorAll('.comment-item').forEach(item => {
        item.addEventListener('click', function(e) {
            if (e.target.closest('.comment-like-btn') || e.target.closest('.comment-image-item')) return;
            const unameEl = this.querySelector('.comment-username');
            if (!unameEl) return;
            const replyName = unameEl.textContent.trim();
            if (!userInfo) {
                showModal('loginModal');
                return;
            }
